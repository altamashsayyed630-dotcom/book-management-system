...
function EditBook() {
    ...
    const fetchBook = async () => {
      try {
        ...
        setBook({
          ...
          genre: d.genre || "", publicationYear: d.publicationYear || "",
          phone: d.phone || "", email: d.email || "",
        });
        if (d.image) { setExistingImage(d.image); setImagePreview(d.image); }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await API.put(`/books/${id}`, { ...book, image: imageUrl });
      alert("Book Updated Successfully!");
      navigate("/books");
    } catch { setError("Failed to update book."); setUploadProgress(""); }
    finally { setSaving(false); }
  };

  if (loading) return (
    <div className="page"><div className="card">
      <div className="state-box"><div className="spinner"></div><p>Loading book...</p></div>
    </div></div>
  );
    ...
    <div className="page">
      <div className="card">
        ...
        <form onSubmit={handleSubmit} className="book-form">
          ...
          {/* Photo Upload */}
          <div className="form-group">
            <label>{imagePreview ? "Change Cover Photo" : "Add Cover Photo"} (optional)</label>
              ...
              {imagePreview && (
                <div className="image-preview-wrap">
                  ...
                  <div className="preview-info">
                    <p className="preview-name">{imageFile ? imageFile.name : "Current cover"}</p>
                    <button type="button" className="remove-image-btn"
                      onClick={() => { setImagePreview(null); setImageFile(null); setExistingImage(""); }}>
                      ✕ Remove
                    </button>
                  </div>
                </div>
